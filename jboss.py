...
class CannalPOC(POCBase):
    ...
    def _check(self):
        ...
        try:
            url = self.url.strip() + "/jmx-console/HtmlAdaptor?action=displayMBeans"  # self.url 就是你指定的-u 参数的值
            res = requests.get(url=url, verify=False, timeout=9)
            if 'JBoss JMX Management Console' in res.content.decode() and res.status_code == 200 and 'jboss' in res.content.decode():
